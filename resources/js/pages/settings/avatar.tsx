...
import { Camera, ZoomIn, ZoomOut, Loader2, Upload, AlertCircle, Check, X } from 'lucide-react';
...
export default function AvatarSettings() {
    ...
    // 内存安全清理机制：切换或重置图片时释放原生 ObjectURL
    useEffect(() => {
        return () => {
                ...
                {imageSrc && (
                    <div className="rounded-2xl border border-border/80 bg-card p-5 shadow-xs space-y-4 animate-in fade-in slide-in-from-top-4 duration-200">
                        <div className="flex items-center justify-between border-b border-border/60 pb-3">
                            <h4 className="font-medium text-sm text-foreground">调整头像尺寸与范围</h4>
                            <Button
                                type="button"
                                variant="ghost"
                                size="sm"
                        ...
                        {/* 缩放条控制区 */}
                        <div className="flex items-center gap-3 px-2 py-1 bg-muted/40 rounded-xl border border-border/40">
                            <button
                                type="button"
