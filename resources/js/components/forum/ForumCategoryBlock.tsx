...
export default function ForumCategoryBlock({ category }: { category: ForumCategory }) {
    ...
    return (
        <div className="w-full bg-card rounded-2xl border border-border/80 shadow-sm overflow-hidden mb-6">
            {/* 分类标题栏 */}
            <div
                className={`flex items-center justify-between px-5 py-3.5 bg-muted/60 cursor-pointer select-none hover:bg-muted/80 transition-colors ${isCollapsed ? 'border-b-0' : 'border-b border-border/80'
                    }`}
